...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		if (!rs.isNull("userlogin")) {
			c = new PdfPCell(new Phrase("Login", tblHeaderFont));
			c.setGrayFill(0.95f);
		...
		if (!rs.isNull("ip")) {
			c = new PdfPCell(new Phrase("IP remota comienza por", tblHeaderFont));
			c.setGrayFill(0.95f);
			c.setHorizontalAlignment(Element.ALIGN_LEFT);
			datatable.addCell(c);
	...
	PdfPTable getDataTable(GenericTransaction data) throws Throwable
	{
		...
		c = new PdfPCell( new Phrase("IP remota", tblHeaderFont) );
		c.setGrayFill(0.95f);
		c.setHorizontalAlignment(Element.ALIGN_CENTER);
		datatable.addCell(c);
		...
		while (rs.next())
		{
			...
			v = rs.getString("login_date");
			c = new PdfPCell( new Phrase( v, tblBodyFont ) );
			c.setHorizontalAlignment(Element.ALIGN_CENTER);
			datatable.addCell(c);
			
			v = rs.getString("login_time");
			c = new PdfPCell( new Phrase( v, tblBodyFont ) );
			c.setHorizontalAlignment(Element.ALIGN_CENTER);
			datatable.addCell(c);
			
			v = rs.getString("remote_addr");
			c = new PdfPCell( new Phrase( v, tblBodyFont ) );
			c.setHorizontalAlignment(Element.ALIGN_CENTER);
			datatable.addCell(c);
