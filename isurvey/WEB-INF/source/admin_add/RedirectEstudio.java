...
public class RedirectEstudio extends GenericTransaction {
	   ...
	   public int service(Recordset inputParams) throws Throwable {
	       ...
	       if (getTipoEstudio(id).equals("Cerrado"))
	    	   getRequest().setAttribute("ruta", "action/administrador_add/monitoreo_general/estudio/cerrado/instrumentos/form?id=" + id);
	       if (getTipoEstudio(id).equals("Abierto-Identificado"))
	    	   getRequest().setAttribute("ruta", "action/administrador_add/monitoreo_general/estudio/abierto_identificado/form?id=" + id);
	       if (getTipoEstudio(id).equals("Abierto-Anonimo"))
	    	   getRequest().setAttribute("ruta", "action/administrador_add/monitoreo_general/estudio/abierto_anonimo/form?id=" + id);
	       return 0;
	   }
