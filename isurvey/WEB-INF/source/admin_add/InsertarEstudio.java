 ...
 * @author addSolutions
 */
public class InsertarEstudio extends GenericTransaction {
    ...
    public int service(Recordset inputParams) throws Throwable{
        ...
        if (tipo.equals("Abierto-Identificado")){
            ...
            String sql3 = StringUtil.replace(getResource("insert.sql"), "{{nombre_estudio}}", nombre);
            sql3 = StringUtil.replace(sql3, "{{id_empresa}}", idEmpresa);
            sql3 = StringUtil.replace(sql3, "{{tipo}}", tipo);
            getDb().exec(sql3);
